...
class TestUpdateEntries(TestCase):
    ...
    def setUpClass(cls) -> None:
        """
        Create new testuser and new testgenre
        in database to run tests on
        """
    ...
    def test_edit_book(self, client):
        ...
        client.post(
            "/edit_book/{}".format(
                testbook.id), data={'status': 'complete',
                                    'favourite': True,
                                    'book_genre': 1,
                                    'isbn': None,
                                    'review':
                                    'testauthor is not so bad'})
    ...
    def test_edit_genre(self, client):
        ...
        genre = db.session.query(Genre).filter(
            Genre.genre_name == "testgenre",
            Genre.genre_owner == "testuser").one()
        client.post("/edit_genre/{}".format(genre.id),
                    data={'genre_name': 'horror'})
        updated_genre = db.session.query(
            Genre).filter(Genre.id == genre.id).one()
        self.assertEqual(updated_genre.genre_name, "horror")

    def test_no_duplicate_genres(self, client):
        """
        Confirm that /edit_genre route does not
        update database entry if genre name already exists
        """
        client.post(
            "/sign_in", data={"username": "testuser",
                              "password": "testpass"})
        genre = db.session.query(Genre).filter(
            Genre.genre_owner == "testuser").one()
        # Following code snippet to test for flash messages on the page
        # taken from suggestion of stackoverflow user Leo Skhrnkv,
        # linked in readme credits
        response = client.post("/edit_genre/{}".format(genre.id),
                               data={'genre_name': 'horror'},
                               follow_redirects=True)
        self.assertIn("That genre already exists in your library!",
                      response.get_data(as_text=True))

    def test_edit_auth_fail(self, client):
        """
        Confirm that /edit_genre route does not
        update database entry when user not logged into session
        """
        genre = db.session.query(Genre).filter(
            Genre.genre_name == "testgenre",
            Genre.genre_owner == "testuser").one()
        client.post("/edit_genre/{}".format(genre.id),
                    data={'genre_name': 'Horse Books'})
        updated_genre = db.session.query(
            Genre).filter(Genre.id == genre.id).one()
...
class TestSaveBooks(TestCase):
    ...
    all books in a genre to the default misc
    """
    ...
    def test_save_book(self, client):
        """
        Confirm that /save_books changes genre to misc
        for all books in the genre passed to the route
        """
        client.post(
            "/sign_in", data={"username": "testuser",
                              "password": "testpass"})
        book = db.session.query(Book).filter(
            Book.book_title == "testbook").one()
        current_genre = db.session.query(Genre).filter(
            Genre.id == book.book_genre).one()
        ...
        updated_genre = db.session.query(Genre).filter(
            Genre.genre_name == "misc", Genre.genre_owner == "testuser").one()
